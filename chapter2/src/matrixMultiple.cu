...
int main(int argc, char **argv)
{
	...
	cudaMemcpy(deviceRef, d_C, Cxy * sizeof(float), cudaMemcpyDeviceToHost);
	printf("Matrix_deviceRef shem: (%d×%d) blocksize: %d,  <<<(%d,%d),(%d,%d)>>>  GPU运行时间为：%fs\n",
		M, N, BLOCK_SIZE, grid.x, grid.y, block.x, block.y, elapsedTime / 1000);
	//printMatrix(deviceRef, M, N);
/*
        elapsedTime = 0.0;
        cudaEventCreate(&gpustart);
...
*/
        ...
        printf("Matrix_deviceRef cublas: (%d×%d)  <<<(%d,%d),(%d,%d)>>>  GPU运行时间为：%fs\n",
                M, N, grid.x, grid.y, block.x, block.y, elapsedTime / 1000);
